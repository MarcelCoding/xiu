    ...
    loop {
      match self.state {
        ...
        ClientSessionState::Connect => {
          ...
          self
            .send_connect(&(define::TRANSACTION_ID_CONNECT as f64))
            .await?;
        ...
        ClientSessionState::CreateStream => {
          ...
          self
            .send_create_stream(&(define::TRANSACTION_ID_CREATE_STREAM as f64))
            .await?;
          self.state = ClientSessionState::WaitStateChange;
        }
        ClientSessionState::Play => {
          log::info!("[C -> S] Play...");
          self
            .send_play(&0.0, &self.stream_name.clone(), &0.0, &0.0, &false)
            .await?;
          self.state = ClientSessionState::WaitStateChange;
        }
        ClientSessionState::PublishingContent => {
          ...
          self
            .send_publish(&0.0, &self.stream_name.clone(), &"live".to_string())
            .await?;
          self.state = ClientSessionState::WaitStateChange;
        }
  ...
  pub async fn process_messages(
    ...
    match msg {
      RtmpMessageData::Amf0Command {
        ...
        self
          .on_amf0_command_message(command_name, transaction_id, command_object, others)
          .await?
      }
