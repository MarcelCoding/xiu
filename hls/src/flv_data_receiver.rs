use {
  ...
  flv::define::FlvData,
  rtmp::channels::define::{ChannelData, ChannelDataConsumer, ChannelEvent, ChannelEventProducer},
  rtmp::session::{
    common::SessionInfo,
...
impl FlvDataReceiver {
  ...
  pub async fn run(&mut self) -> Result<(), HlsError> {
    self
      .subscribe_from_rtmp_channels(self.app_name.clone(), self.stream_name.clone())
      .await?;
